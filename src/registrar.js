...
export default class Registrar {
  ...
  async getPriceCurve() {
    try {
      return this.getText('plq', 'oracle')
    } catch (e) {
  ...
  async makeCommitment(name, owner, secret = '') {
    ...
    const account = await getAccount()
    const resolverAddr = await this.getAddress('resolver.plq')
  ...
  async submitProof(name, parentOwner) {
    ...
    if (data.length === 0) {
      ...
      if (!isOld && owner === user) {
        const resolverAddress = await this.getAddress('resolver.plq')
...
async function getEthResolver(ENS) {
  const resolverAddr = await ENS.resolver(namehash('plq'))
  const provider = await getProvider()
...
export async function setupRegistrar(registryAddress) {
  ...
  let ethAddress = await ENS.owner(namehash('plq'))
